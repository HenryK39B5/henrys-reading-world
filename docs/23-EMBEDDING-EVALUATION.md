...
迁移后首次重跑标签 query vectors 与 300 条试标：当时标签覆盖仍为 53 / 53，0 孤儿、0 过宽、3 偏薄；多标签比例从 51.7% 变为 51.0%，属于量化与 pooling 实现差异，没有改变产品判断。Batch 3 于 2026-09-19 完成词表缺口收口后已扩为 56 个标签，最终试标数据以 `docs/25` 为准。
