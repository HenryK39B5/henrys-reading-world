...
> 状态：交给实现模型的当前施工路线。  
> 产品依据：`docs/10-PRODUCT-DIRECTION-V2.md`。  
> 原则：按片迁移、每片测试并记录；不要把全量数据、schema、算法与 UI 一次性全部改完后才排错。

## 0. 当前基线与迁移目标

当前 commit 基线：`d9ceeb0`。工作区应先保持干净。

v1 已有：46 条 / 20 本 / 5 个逐句主题，Opening/Contrast/Surprise 算法，出处、书、主题展开、年份、About、真实封面与取色；86 单测、23 E2E。
...
在句子上方或控制区附近增加低权重 scope 行：

- all：`随便看看` + `换个主题`
- theme：`正在逛：<主题>` + `换个主题` + `回到随便看看`

主题说明可写 `从这个主题书架中的书随机抽取`。不要在每句旁显示多个 tag，不显示算法 reason，不增加主题大色块。`再来一句` 文案可继续沿用。

主题选择可使用轻量 popover/dialog 或舞台下的主题列表；必须键盘可达、Esc 可关闭、焦点可恢复。选中后滚回并聚焦舞台，尊重 reduced-motion。
...
删除访客文案 `反复出现的问题` 和“只呈现材料，不替主人下结论”。主题书架每行/块显示标题、平实说明、真实 `N 本书 · M 处划线`，最多展示 3～4 张小封面；点击进入主题舞台，不直接展开 10～12 条文本。

### 5.3 年份

年份控件移入书籍区，只过滤书籍列表和打开的单书顺序列表；不改变主题计数、主题可用书或主题舞台。若当前只有 3 个年份，保持低视觉重量。

### 5.4 书籍与全量划线

- 书籍列表初始 8～12 本，之后分批增加；避免一次加载/展示 127 张原始封面。
- 单书详情初始 8～12 条，`再显示 20 条`，直到全部可达；不要 `查看全部 157 处` 后一次渲染。
- 保留 `随机看一处`，从该书全量划线中公平防重抽取并回舞台。
- 暂不增加全文搜索、复杂排序和虚拟列表；真实页面性能不足时再升级。

### 5.5 About

只显示真实范围和可选的 Henry 自写介绍。删除解释“不统计时长、不做排行榜、不评价阅读者”的防御性段落。
...
- 增加 scope UI；主题列表点击进入并持久停留。
- 主题书架移到书籍之前；主题不再展开划线墙。
- 年份收进书籍；About 精简。

验收：all → theme A 连续 3 次 → theme B → all → source/book → 回原 stage scope 全流程；桌面/390px 截图；键盘和焦点。

### V2-D — 全量内容浏览

- 书籍与单书内容分批显示；同书随机入口。
- 检查 130 本、4,663 条下 DOM 数量、交互响应与封面加载。
- 确保每条 highlight 至少可通过对应书详情到达；测试不要尝试在一个页面同时渲染全部。

验收：最多划线书可逐批走到末尾；末批数量正确；无横向溢出；页面初始 DOM 不含数千条 passage。

### V2-E — 原 Slice 5

在 v2 状态和 schema 稳定后再实现：稳定 `?h=`、复制、4:5 分享预览、失败回退、200% 缩放、完整键盘和错误状态。分享只锁定 highlight ID，不把临时主题 scope 当作内容身份。
...
6. 遇到真正产品冲突才问用户，不为已确定的全量开发许可、主题书架语义或公平抽样重复询问。
