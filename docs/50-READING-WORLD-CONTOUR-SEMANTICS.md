...
## Updated exploration boundary (2026-09-25)

Henry considers the region-view seven-level contours acceptable and permits broader 2D studies, including isolated local embedding/UMAP experiments; the earlier restrictions in this document described the scope of the first fixed-point comparison, not a permanent ban. The controlled density and renderer study is in `docs/51-DENSITY-RESOLUTION-AND-RENDERING-STUDY.md`. The subsequent `docs/52-MAP-SURFACE-REFERENCE-STUDY.md` separates coordinate marks from density-cell texture: small crosses mark fixed coordinate intersections, not density samples or clickable highlights; a quiet continuous grid is only a static-share composition candidate. No exploratory candidate is automatically accepted for the public map, and remote embedding remains subject to separate permission.
