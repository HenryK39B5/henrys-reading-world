...
>
> Release-A 及两轮公开审核均已完成；私有清单为 108 本公开、22 本排除、6 条单独排除。Release-B 因 V3 产品迭代暂停。Batch 0–3 已完成：默认模型已覆盖全部 4,663 条，53 个稳定标签与 300 条试标已建立，Local Tag Studio 可用。实际状态见 `docs/23–25`。
...
3. **Batch 2**：已完成；全量 embedding、300 条按书公平样本、53 个候选标签与人工种子；用户已批准全部候选进入试标。
4. **Batch 3**：已完成到用户内容 Gate；53 个稳定标签、严格 assignment 契约、300 条试标（268 reviewed / 32 draft）与 Local Tag Studio 均已完成。
5. **Batch 4**：用户批准试标后才开始 schema 3、主题小径、岔路、分享全部标签与关键视觉原型。
