...
- `discovery.test.ts`：删掉钉住“首屏按长度筛书”的旧用例，改为“公平抽中的书自己没有任何可读划线也照常开局”与“长度带外的 5 本书都能在首屏被抽到”；把“留在当前书”的用例拆成“有别的书就轮换”与“只有一本书时才留在本书”。
...
1. 首屏最坏 299 字（一本只有 1 条划线、且该划线 299 字的真实书）。是否允许，属产品口味，待复核。
2. 公开 bundle 中保留 `local-covers/` 路径前缀字符串（本机封面路由分支）；无封面文件、无真实内容、无私人字段。早期记录措辞已改正。
3. 主题/房间 UI 与 Book Aura 仍属 V2-C1 / V2-C2。
