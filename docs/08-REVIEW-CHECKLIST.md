...
| V2-B 公平发现算法 | 已完成 | 两阶段公平引擎 + all/theme 持久 scope + 分 scope cycle；真实数据首次 130 次抽完全部 130 本且 0 重复；详见下方 V2-B 执行记录 |
...
| 40 次开局抽 | 36 本不同书，长度 24–118 字（可读 40/40） |
...
| `npm run build` | 成功；dist 无 `local-covers` / planIndex / bookmarkId / deepLink / 密钥标记 |
| `npx vite build --mode local-private` | 按预期拒绝，未生成产物 |
...
1. **“没见过优先”高于“每本书均等轮次”**。当某本书的划线已经全部展示、而当前书还有没看过的划线时，引擎会留在当前书而不是回头重抽那本小书。这是 docs/10 §6.6（避免重复）优先于 §6.2（避免相邻同书）的有意取舍：真实数据 600 次连续抽取得到 600 条不重复划线，代价是单本出现次数为 1–6 次（按轮次看是 4.6 轮），最大书占 1.00%。若用户希望“严格每轮每本一次、小书允许重复”，只需把 fresh-with-unseen 与 fresh 两层合并，属产品口味而非缺陷，留待 review 决定。
