...
test.describe('rooms and their URLs', () => {
    ...
    test('about keeps real excerpts distinct from complete context or personal endorsement', async ({ page }) => {
        ...
        const rights = page.getByTestId('about-rights');
        await expect(rights).toContainText('个人阅读记录中的原文摘录');
        await expect(rights).toContainText('不代表作者、出版社或微信读书的立场');
        await expect(rights).toContainText('书名、作者信息与书封属于各自权利人');
