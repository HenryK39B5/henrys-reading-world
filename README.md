...
- 公开快照 `src/data/public-snapshot.json` 已包含获准内容；`npm run dev` 可本机预览公开范围，`npm run dev:local` 可回看完整本地范围。新增 `/design` 页面向访客解释网站的阅读方式、主题、地图与公开边界。导出和构建不代表已部署。
...
`dev:local` 读取完整的 `.private/local-snapshot.json`，顶部标出“仅本机 · 未公开审核”；普通 `dev` 只读取已导出的公开快照，两者均展示真实划线。
...
npx playwright test --config playwright.capture-cross-page.config.ts # 7F 同数据跨页截图，仅写 .private/
npx playwright test --config playwright.audit-7g.config.ts # 7G 130本/56小径全量边界审计，仅写 .private/
npx playwright test --config playwright.audit-7h.config.ts # 7H 对比度/键盘路线审计，仅写 .private/
...
npm run test:public         # 已导出非空公开快照的浏览器验收
```
...
公开路径（读取已审核的真实内容，仅在本机预览；尚未部署）：

```powershell
npm run dev                 # 只读取获准导出的 src/data/public-snapshot.json
